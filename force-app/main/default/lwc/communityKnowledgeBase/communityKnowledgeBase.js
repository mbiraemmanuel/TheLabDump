...
export default class CommunityKnowledgeBase extends NavigationMixin(LightningElement) {
    @api menuName;
    @api buttonLabel;
    @api buttonRedirectPageAPIName;

    @api title = 'Knowledge Base';
    @api descriptionText = 'Find Answers. Get Support. Stay Current.';
    @api additionalInfoText = 'Select a category below to explore our knowledge base resources.';
        ...
        getNavigationMenuItems({ 
            ...
            .then(data => {
                ...
                this.rawMenuItems = data.map((item, index) => {
                    ...
                    // Get the refined label for the icon path
                    const refinedLabel = this.refineLabel(item.Label);
                    
                    return {
                        id: index,
                        label: labelForDisplay,
                        icon: NavigationImages + '/Icons/' + refinedLabel + '.png',
                        target: item.Target,
