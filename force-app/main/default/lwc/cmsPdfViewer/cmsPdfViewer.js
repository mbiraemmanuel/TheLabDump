...
export default class CmsPdfViewer extends LightningElement {
  ...
  async init() {
    ...
    try {
      ...
      console.log('Extracted content key:', key);
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onload = () => {
        ...
        if (pdfjsLib) {
          ...
          // Disable worker for LWR compatibility
  ...
  getContentKey() {
    if (typeof this.managedContent === 'string' && this.managedContent) {
      console.log('Using managedContent:', this.managedContent);
      return this.managedContent;
    }
    if (this.fallbackContentKey) {
      console.log('Using fallbackContentKey:', this.fallbackContentKey);
      return this.fallbackContentKey;
    }
    
    const path = window.location.pathname || '';
    console.log('Parsing pathname for content key:', path);
    
    // Try Aura site pattern: /cms-document/[title]-[KEY]
    let i = path.indexOf('/cms-document/');
    if (i >= 0) {
      const tail = path.substring(i + 14).replace(/\/+$/, '');
      const j = tail.lastIndexOf('-');
      const key = j > -1 ? tail.substring(j + 1) : tail;
      console.log('Found Aura pattern, extracted key:', key);
      return key;
    }
    
    // Try LWR site pattern: /document/[title]-[KEY]
    i = path.indexOf('/document/');
    if (i >= 0) {
      const tail = path.substring(i + 10).replace(/\/+$/, '');
      console.log('Found LWR pattern, tail:', tail);
      // Remove query parameters if any
      const withoutQuery = tail.split('?')[0];
      const j = withoutQuery.lastIndexOf('-');
      const key = j > -1 ? withoutQuery.substring(j + 1) : withoutQuery;
      console.log('Extracted LWR key:', key);
      return key;
    }
